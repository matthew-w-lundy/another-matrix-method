...
on_runlist = ReadRunListFromFile(
    "../easy-matrix-method/output_vts_hours/RunList_UrsaMinor_V6.txt"
)
src_ra = 227.2854167
src_dec = 67.2225000
...
for run in range(0, total_runs):
    run_all_sky_map, run_data_xyoff_map, run_fit_xyoff_map = build_skymap(
        smi_input, [on_runlist[run]], src_ra, src_dec
    )
    for logE in range(0, logE_bins):
        ...
        for gcut in range(0, gcut_bins):
            for idx_x in range(0, xoff_bins):
                for idx_y in range(0, yoff_bins):
                    ...
                    err_xyoff_map[logE].waxis[idx_x, idx_y, gcut] = (
                        data - model
                    ) / data_err
        ...
        smooth_image(
            data_sky_map_smooth[logE].waxis[:, :, 0],
            data_sky_map_smooth[logE].xaxis,
            data_sky_map_smooth[logE].yaxis,
        )
        smooth_image(
            bkgd_sky_map_smooth[logE].waxis[:, :, 0],
            bkgd_sky_map_smooth[logE].xaxis,
            bkgd_sky_map_smooth[logE].yaxis,
        )
        for idx_x in range(0, skymap_bins):
            for idx_y in range(0, skymap_bins):
    ...
    for logE in range(0, logE_bins):
        ...
        for gcut in range(0, gcut_bins):
            ...
            im = axbig.imshow(
                data_xyoff_map[logE].waxis[:, :, gcut].T,
                origin="lower",
                extent=(xmin, xmax, ymin, ymax),
                aspect="auto",
            )
            cbar = fig.colorbar(im)
            fig.savefig(
                f"output_plots/xyoff_map_logE{logE}_gcut{gcut}_data.png",
                bbox_inches="tight",
            )
            axbig.remove()
            ...
            im = axbig.imshow(
                fit_xyoff_map[logE].waxis[:, :, gcut].T,
                origin="lower",
                extent=(xmin, xmax, ymin, ymax),
                aspect="auto",
            )
            cbar = fig.colorbar(im)
            fig.savefig(
                f"output_plots/xyoff_map_logE{logE}_gcut{gcut}_fit.png",
                bbox_inches="tight",
            )
            axbig.remove()
            ...
            im = axbig.imshow(
                err_xyoff_map[logE].waxis[:, :, gcut].T,
                origin="lower",
                extent=(xmin, xmax, ymin, ymax),
                vmin=-max_z,
                vmax=max_z,
            ...
            cbar = fig.colorbar(im)
            fig.savefig(
                f"output_plots/err_xyoff_map_logE{logE}_gcut{gcut}.png",
                bbox_inches="tight",
            )
            axbig.remove()
