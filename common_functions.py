...
run_elev_cut = 25.0
max_MeanPedvar_cut = 11.0
min_MeanPedvar_cut = 3.0
if "output_hnsb" in smi_output:
    min_MeanPedvar_cut = 7.0
if "output_lnsb" in smi_output:
    max_MeanPedvar_cut = 5.0

# min_NImages = 2
min_NImages = 3
max_Roff = 1.7
max_EmissionHeight_cut = 20.0
min_EmissionHeight_cut = 6.0
max_Rcore = 400.0
min_Rcore = 0.0
min_Energy_cut = 0.02
...
calibration_radius = (
    0.15  # need to be larger than the PSF and smaller than the integration radius
)
...
if "free" in norm_tag:
    ...
    fov_mask_radius = float(norm_tag.strip("fov")) / 10.0

gcut_start = 0
gcut_end = gcut_bins
gcut_weight = [1.0] * gcut_bins

Normalized_MSCL_cut = []
Normalized_MSCW_cut = []
if gcut_bins == 6:
    Normalized_MSCL_cut += [1.0]
    Normalized_MSCL_cut += [3.0]
    Normalized_MSCW_cut += [1.0]
    Normalized_MSCW_cut += [3.0]
    Normalized_MSCW_cut += [5.0]
elif gcut_bins == 8:
...
if "nbin0" in bin_tag:
    for logE in range(0, logE_nbins):
        xoff_bins[logE] = 9
        yoff_bins[logE] = 9
        if pow(10.0, logE_bins[logE]) > 0.4:
            xoff_bins[logE] = 7
            yoff_bins[logE] = 7
        if pow(10.0, logE_bins[logE]) > 1.0:
            xoff_bins[logE] = 5
            yoff_bins[logE] = 5
        if pow(10.0, logE_bins[logE]) > 3.0:
            xoff_bins[logE] = 3
            yoff_bins[logE] = 3
        if pow(10.0, logE_bins[logE]) > 10.0:
            xoff_bins[logE] = 1
            yoff_bins[logE] = 1
...
def weighted_least_square_solution(mtx_input, vec_output, vec_weight, plot_tag=""):
    ...
    ax.set_yscale("log")
    fig.savefig(
        f"output_plots/least_square_sigularvalue_{plot_tag}.png", bbox_inches="tight"
    )
    del fig
    del ax
    plt.close()
...
def significance_li_and_ma(N_on, N_bkg, N_bkg_err):
    ...
    on_nlogn = 0.0
    if N_on > 0.0:
        on_nlogn = N_on * np.log(2.0 * (N_on / (N_on + N_bkg)))
    bkg_nlogn = 0.0
    if N_bkg > 0.0:
        bkg_nlogn = N_bkg * np.log(2.0 * (N_bkg / (N_on + N_bkg)))

    chi_square_stat = 2 * (on_nlogn + bkg_nlogn)
    chi_square = chi_square_stat

    if abs(N_bkg_err) > 0.0:
        chi_square_syst = pow((N_on - N_bkg) / N_bkg_err, 2)
        chi_square = 1.0 / (1.0 / chi_square_stat + 1.0 / chi_square_syst)
...
def GetRunElevAzim(smi_input, run_number):
    ...
    TelTree.GetEntry(int(float(TelTree.GetEntries()) / 2.0))
...
def ReadOffRunListFromFile(
    smi_input, input_onlist_file, input_offlist_file, mimic_index
):
...
def smooth_image(image_data, xaxis, yaxis, kernel_radius=0.07):
    ...
    if kernel_pix_size == 0:
        return

    gaus_norm = 2.0 * np.pi * kernel_radius * kernel_radius
    image_kernel = np.zeros_like(image_data)
    central_bin_x = int(float(len(xaxis) - 1) / 2.0)
    central_bin_y = int(float(len(yaxis) - 1) / 2.0)
    for idx_x in range(0, len(xaxis) - 1):
        for idx_y in range(0, len(yaxis) - 1):
            ...
            pix_content = np.exp(
                -(distance * distance) / (2.0 * kernel_radius * kernel_radius)
            )
    ...
    for idx_x1 in range(0, len(xaxis) - 1):
        for idx_y1 in range(0, len(yaxis) - 1):
            image_smooth[idx_y1, idx_x1] = 0.0
            for idx_x2 in range(
                idx_x1 - 3 * kernel_pix_size, idx_x1 + 3 * kernel_pix_size
            ):
                for idx_y2 in range(
                    idx_y1 - 3 * kernel_pix_size, idx_y1 + 3 * kernel_pix_size
                ):
                    if idx_x2 < 0:
                        continue
                    if idx_y2 < 0:
                        continue
                    if idx_x2 >= len(xaxis) - 1:
                        continue
                    if idx_y2 >= len(yaxis) - 1:
                        continue
                    if central_bin_y + idx_y2 - idx_y1 < 0:
                        continue
                    if central_bin_x + idx_x2 - idx_x1 < 0:
                        continue
                    if central_bin_y + idx_y2 - idx_y1 >= len(yaxis) - 1:
                        continue
                    if central_bin_x + idx_x2 - idx_x1 >= len(xaxis) - 1:
                        continue
                    old_content = image_data[idx_y2, idx_x2]
                    scale = image_kernel[
                        central_bin_y + idx_y2 - idx_y1, central_bin_x + idx_x2 - idx_x1
                    ]
...
class MyArray3D:
            ...
            for idx_y in range(0, len(self.yaxis) - 1):
                for idx_z in range(0, len(self.zaxis) - 1):
                    self.waxis[idx_x, idx_y, idx_z] = (
                        self.waxis[idx_x, idx_y, idx_z] * factor
                    )
    ...
    def addSquare(self, add_array, factor=1.0):
        for idx_x in range(0, len(self.xaxis) - 1):
            for idx_y in range(0, len(self.yaxis) - 1):
                for idx_z in range(0, len(self.zaxis) - 1):
                    self.waxis[idx_x, idx_y, idx_z] = pow(
                        pow(self.waxis[idx_x, idx_y, idx_z], 2)
                        + pow(add_array.waxis[idx_x, idx_y, idx_z] * factor, 2),
                        0.5,
                    )
    ...
    def get_bin(self, value_x, value_y, value_z):
        ...
        key_idx_z = -1
        for idx_x in range(0, len(self.xaxis) - 1):
            if abs(self.xaxis[idx_x] - value_x) <= abs(self.delta_x) and abs(
                self.xaxis[idx_x + 1] - value_x
            ) < abs(self.delta_x):
                key_idx_x = idx_x
                break
        for idx_y in range(0, len(self.yaxis) - 1):
            if abs(self.yaxis[idx_y] - value_y) <= abs(self.delta_y) and abs(
                self.yaxis[idx_y + 1] - value_y
            ) < abs(self.delta_y):
                key_idx_y = idx_y
                break
        for idx_z in range(0, len(self.zaxis) - 1):
            if abs(self.zaxis[idx_z] - value_z) <= abs(self.delta_z) and abs(
                self.zaxis[idx_z + 1] - value_z
            ) < abs(self.delta_z):
                key_idx_z = idx_z
                break
...
class MyArray1D:
    ...
    def __init__(self, x_nbins=10, start_x=0.0, end_x=10.0, x_bins=[], overflow=False):
        if len(x_bins) == 0:
            array_shape = x_nbins
    ...
    def get_bin(self, value_x):
        key_idx_x = -1
        for idx_x in range(0, len(self.xaxis) - 1):
            if abs(self.xaxis[idx_x] - value_x) <= abs(self.delta_x[idx_x]) and abs(
                self.xaxis[idx_x + 1] - value_x
            ) < abs(self.delta_x[idx_x]):
                key_idx_x = idx_x
                break
    ...
    def fill(self, value_x, weight=1.0):
        key_idx = self.get_bin(value_x)
        if key_idx == -1:
            key_idx = 0
            weight = 0.0
        self.waxis[key_idx] += 1.0 * weight

    def divide(self, add_array):
        for idx_x in range(0, len(self.xaxis) - 1):
            if add_array.waxis[idx_x] == 0.0:
                self.waxis[idx_x] = 0.0
            else:
...
def GetGammaSources(tele_point_ra, tele_point_dec):
    ...
    for line in inputFile:
        ...
        distance = pow(
            pow(star_ra - tele_point_ra, 2) + pow(star_dec - tele_point_dec, 2), 0.5
        )
        if distance > 3.0:
            continue
        # print (f'{line_split}')
...
def CoincideWithBrightStars(ra, dec, bright_stars_coord):
    ...
    for star in range(0, len(bright_stars_coord)):
        ...
        if distance > bright_star_radius_cut:
            continue
        isCoincident = True
    return isCoincident
...
def convert_multivar_vector1d_to_map3d(multivar_map_1d):
    ...
    for logE in range(0, logE_nbins):
        xyoff_map += [
            MyArray3D(
                x_bins=xoff_bins[logE],
                start_x=xoff_start,
                end_x=xoff_end,
                y_bins=yoff_bins[logE],
                start_y=yoff_start,
                end_y=yoff_end,
                z_bins=gcut_bins,
                start_z=gcut_start,
        ...
        xyvar_map += [
            MyArray3D(
                x_bins=xvar_bins[logE],
                start_x=-1.0,
                end_x=end_x,
                y_bins=yvar_bins[logE],
                start_y=-1.0,
                end_y=end_y,
                z_bins=1,
                start_z=0.0,
    ...
    for gcut in range(0, gcut_bins):
        for logE in range(0, logE_nbins):
            for idx_x in range(0, xoff_bins[logE]):
                for idx_y in range(0, yoff_bins[logE]):
                    idx_1d += 1
                    xyoff_map[logE].waxis[idx_x, idx_y, gcut] = multivar_map_1d[
                        idx_1d - 1
                    ]
    for logE in range(0, logE_nbins):
        for idx_x in range(0, xvar_bins[logE]):
...
def build_big_camera_matrix(
    ...
    for run_number in runlist:
        ...
        TelTree = InputFile.Get(TreeName)
        TelTree.GetEntry(int(float(TelTree.GetEntries()) / 2.0))
        TelRAJ2000 = TelTree.TelRAJ2000 * 180.0 / np.pi
        TelDecJ2000 = TelTree.TelDecJ2000 * 180.0 / np.pi
        TelElevation = TelTree.TelElevation
        TelAzimuth = TelTree.TelAzimuth
        ...
        if not is_bkgd:
            ...
            if avg_MeanPedvar > max_MeanPedvar_cut:
                continue
            if avg_MeanPedvar < min_MeanPedvar_cut:
                continue
        ...
        for entry in range(0, total_entries):
            ...
            if NImages < 2:
                continue
            if not ApplyTimeCuts(Time - Time0, list_timecuts):
                continue
            if logE < 0:
                continue
            if logE >= len(xyoff_map):
                continue
            if Energy < min_Energy_cut:
                continue
            if Energy > max_Energy_cut:
                continue

            MSCW = EvtTree.MSCW / MSCW_cut[logE]
            MSCL = EvtTree.MSCL / MSCL_cut[logE]
            GammaCut = EventGammaCut(MSCL, MSCW)
            if GammaCut > float(gcut_end):
                continue

            if NImages < min_NImages:
                continue
            if EmissionHeight > max_EmissionHeight_cut:
                continue
            if EmissionHeight < min_EmissionHeight_cut:
                continue
            ...
            if is_bkgd:
                ...
                found_roi = CoincideWithRegionOfInterest(
                    Xsky, Ysky, roi_ra, roi_dec, roi_r
                )
                if found_roi:
        ...
        if is_bkgd:
            for logE in range(0, logE_nbins):
                for gcut in range(0, 1):
                    for idx_x in range(0, xoff_bins[logE]):
                        for idx_y in range(0, yoff_bins[logE]):
                            if xyoff_mask_map[logE].waxis[idx_x, idx_y, gcut] > 0.0:
                                bin_coord = xyoff_mask_map[logE].get_bin_center(
                                    idx_x, idx_y, gcut
                                )
                                bin_idx = xyoff_mask_map[logE].get_bin(
                                    -1.0 * bin_coord[0],
                                    -1.0 * bin_coord[1],
                                    bin_coord[2],
                                )
                                xyoff_map[logE].waxis[idx_x, idx_y, gcut] = xyoff_map[
                                    logE
                                ].waxis[bin_idx[0], bin_idx[1], bin_idx[2]]

        xyoff_map_1d = convert_multivar_map3d_to_vector1d(xyoff_map, xyvar_map)
        xyoff_mask_map_1d = convert_multivar_map3d_to_vector1d(
            xyoff_mask_map, xyvar_mask_map
        )
    ...
    if np.sum(np.array(big_exposure_time)) > 0.0:
...
def prepare_vector_for_least_square(multivar_map_1d):
    ...
    sr_map_1d = [0.0 for logE in range(0, logE_nbins)]
    for gcut in range(0, gcut_bins):
        for logE in range(0, logE_nbins):
            for idx_x in range(0, xoff_bins[logE]):
                for idx_y in range(0, yoff_bins[logE]):
...
def residual_correction_fullspec(
    ...
    for idx_x in range(0, xoff_bins[logE]):
        for idx_y in range(0, yoff_bins[logE]):
            if norm[gcut_bins - 1] == 0.0:
                continue
            old_bkgd = bkgd_xyoff_map.waxis[idx_x, idx_y, 0]
            bkgd_xyoff_map.waxis[idx_x, idx_y, 0] += (
                1.0 * residual[gcut_bins - 1] / norm[gcut_bins - 1] * old_bkgd
            )
...
def cosmic_ray_like_chi2_fullspec(
    ...
    pix_log_likelihood = 0.0
    reg_log_likelihood = 0.0
    ...
    if norm_constraint:
        for logE in range(0, logE_nbins):
            ...
            for gcut in range(0, gcut_bins):
                ...
                log_likelihood = significance_li_and_ma(n_data_gcut, n_expect_gcut, 0.0)
        ...
        for logE in range(0, logE_nbins):
            ...
            for idx_x in range(0, xoff_bins[logE]):
                for idx_y in range(0, yoff_bins[logE]):
                    ...
                    if gcut in blind_regions:
                        if mask > 0.0 or "free" in norm_tag:
                            if use_offruns:
                                n_data = init
                            else:
                                weight = 0.0
                    ...
                    if use_poisson_likelihood:
                        log_likelihood = significance_li_and_ma(n_data, n_expect, 0.0)
...
def ReadSNRTargetListFromCSVFile():
    ...
    with open("SNRcat20221001-SNR.csv", newline="") as csvfile:
        ...
        for row in reader:
            ...
            source_size += [0.5 * float(target_size) / 60.0]
...
def GetGammaSourceInfo():
    ...
    if drawBrightStar:
        ...
        for src in range(0, len(star_name)):
            ...
            other_star_coord += [[src_ra, src_dec, 0.0]]
    ...
    if drawPulsar:
        (
            ...
            target_psr_dist,
            target_psr_age,
        ) = ReadATNFTargetListFromFile("ATNF_pulsar_full_list.txt")
        for src in range(0, len(target_psr_name)):
            ...
            for entry in range(0, len(other_stars)):
                distance = pow(gamma_source_ra - other_star_coord[entry][0], 2) + pow(
                    gamma_source_dec - other_star_coord[entry][1], 2
                )
                if distance < near_source_cut * near_source_cut:
                    near_a_source = True
            if not near_a_source:
                other_stars += [gamma_source_name]
                if target_psr_age[src] / 1000.0 > 1e6:
                    other_stars_type += ["MSP"]
                else:
                    other_stars_type += ["PSR"]
                other_star_coord += [[gamma_source_ra, gamma_source_dec, 0.0]]

    if drawSNR:
        target_snr_name, target_snr_ra, target_snr_dec, target_snr_size = (
            ReadSNRTargetListFromCSVFile()
        )
        for src in range(0, len(target_snr_name)):
            ...
            if not near_a_source:
                ...
                other_star_coord += [
                    [gamma_source_ra, gamma_source_dec, gamma_source_size]
                ]
...
def MakeSkymapCutout(skymap_input, cutout_frac):
    ...
    skymap_cutout = MyArray3D(
        x_bins=new_skymap_bins,
        start_x=new_xsky_start,
        end_x=new_xsky_end,
        y_bins=new_skymap_bins,
        start_y=new_ysky_start,
        end_y=new_ysky_end,
        z_bins=1,
        start_z=gcut_start,
        end_z=gcut_end,
...
def PlotCountProjection(
    ...
    E_min = pow(10.0, logE_bins[logE_min])
    E_max = pow(10.0, logE_bins[logE_max])
    ...
    if coordinate_type == "galactic":
        for star in range(0, len(other_stars)):
            other_star_coord[star][0], other_star_coord[star][1] = (
                ConvertRaDecToGalactic(
                    other_star_coord[star][0], other_star_coord[star][1]
                )
            )
    ...
    other_star_types = []
    other_star_markers = []
    star_range = 0.8 * (xmax - xmin) / 2.0
    source_ra = (xmax + xmin) / 2.0
    source_dec = (ymax + ymin) / 2.0
    n_stars = 0
    for star in range(0, len(other_stars)):
        if abs(source_ra - other_star_coord[star][0]) > star_range:
            continue
        if abs(source_dec - other_star_coord[star][1]) > star_range:
            continue
        other_star_markers += [
            [
                other_star_coord[star][0],
                other_star_coord[star][1],
                other_star_coord[star][2],
            ]
        ]
    ...
    make_significance_map(
        hist_map_data,
        hist_map_bkgd,
        hist_map_significance,
        hist_map_excess,
        syst_sky_map=hist_map_syst,
    )
    ...
    x_proj_axis = MyArray1D(
        x_nbins=round(abs(xmax - xmin) / x_pix_size), start_x=xmin, end_x=xmax
    )
    y_proj_axis = MyArray1D(
        x_nbins=round(abs(ymax - ymin) / y_pix_size), start_x=ymax, end_x=ymin
    )
    y_proj_axis_inv = MyArray1D(
        x_nbins=round(abs(ymax - ymin) / y_pix_size), start_x=ymin, end_x=ymax
    )
    ...
    for bx in range(0, len(hist_map_data.xaxis) - 2):
        for by in range(0, len(hist_map_data.yaxis) - 2):
            ...
            for br in range(0, len(x_proj_axis.xaxis) - 1):
                keep_event = False
                if (
                    abs(bin_ra - x_proj_axis.xaxis[br]) <= x_pix_size
                    and abs(bin_ra - x_proj_axis.xaxis[br + 1]) <= x_pix_size
                ):
                    keep_event = True
                if keep_event:
            ...
            for br in range(0, len(y_proj_axis.xaxis) - 1):
                keep_event = False
                if (
                    abs(bin_dec - y_proj_axis.xaxis[br]) <= y_pix_size
                    and abs(bin_dec - y_proj_axis.xaxis[br + 1]) <= y_pix_size
                ):
                    keep_event = True
                if keep_event:
    ...
    for roi in range(0, len(roi_x)):
        mycircle = plt.Circle(
            (roi_x[roi], roi_y[roi]), roi_r[roi], fill=False, color="black"
        )
    ...
    # Plot the histograms
    axHistx.plot(
        x_axis_array, x_bkgd_array, color="r", ls="solid", label="Background model"
    )
    axHistx.errorbar(
        x_axis_array, x_bkgd_array, yerr=x_syst_array, color="r", marker=".", ls="none"
    )
    axHisty.plot(y_bkgd_array, y_axis_array, color="r", ls="solid")
    axHisty.errorbar(
        y_bkgd_array, y_axis_array, xerr=y_syst_array, color="r", marker=".", ls="none"
    )
    axHistx.errorbar(
        x_axis_array,
        x_count_array,
    ...
    txt = axTemperature.text(
        xmax - 0.14, ymax - 0.27, lable_energy_range, fontdict=font
    )
    ...
    # Plot the temperature data
    cax = axTemperature.imshow(
        hist_map_significance.waxis[:, :, layer].T,
        extent=(xmax, xmin, ymin, ymax),
        vmin=-3.0,
        vmax=3.0,
        aspect="auto",
    ...
    for star in range(0, len(other_star_markers)):
        ...
        if other_star_types[star] == "PSR":
            axTemperature.scatter(
                other_star_markers[star][0],
                other_star_markers[star][1],
                s=1.5 * marker_size,
                c=favorite_color,
                marker="+",
                label=other_star_labels[star],
            )
        if other_star_types[star] == "SNR":
            mycircle = plt.Circle(
                (other_star_markers[star][0], other_star_markers[star][1]),
                other_star_markers[star][2],
                fill=False,
                color=favorite_color,
            )
            axTemperature.add_patch(mycircle)
...
def PlotSkyMap(
    ...
    cdict = {
        "red": [
            (0.0, 1.0, 1.0),  # At smallest value -> white (R=1)
            (0.5, 0.0, 0.0),  # At value 2 (scaled to 0.5) -> black (R=0)
            (
                0.5,
                0.2,
                0.2,
            ),  # At value 2 (scaled to 0.5) -> dark red (start of positive)
            (0.75, 1.0, 1.0),  # At some value > 2 -> red
            (1.0, 1.0, 1.0),
        ],  # At largest value -> yellow
        "green": [
            (0.0, 1.0, 1.0),  # White (G=1)
...
def GetFluxCalibration(energy):
    ...
    if flux_calibration[energy] > 0.0:
        return 1.0 / flux_calibration[energy]
    else:
        return 0.0


def make_significance_map(
    data_sky_map, bkgd_sky_map, significance_sky_map, excess_sky_map, syst_sky_map=None
):
    ...
    for idx_x in range(0, skymap_bins):
        for idx_y in range(0, skymap_bins):
            ...
            if norm > 0.0:
                ...
                flux_sky_map.waxis[idx_x, idx_y, 0] = 0.0
                flux_err_sky_map.waxis[idx_x, idx_y, 0] = 0.0
                flux_syst_sky_map.waxis[idx_x, idx_y, 0] = 0.0


def GetRadialProfile(
    hist_flux_skymap,
    hist_error_skymap,
    hist_syst_skymap,
    roi_x,
    ...
    pix_size = (
        abs(
            (hist_flux_skymap.yaxis[1] - hist_flux_skymap.yaxis[0])
            * (hist_flux_skymap.xaxis[1] - hist_flux_skymap.xaxis[0])
        )
        * deg2_to_sr
    )
    bin_size = max(
        radial_bin_scale, 1.0 * (hist_flux_skymap.yaxis[1] - hist_flux_skymap.yaxis[0])
    )
    ...
    for br in range(0, len(radial_axis.xaxis) - 1):
        ...
        for bx in range(0, len(hist_flux_skymap.xaxis) - 1):
            for by in range(0, len(hist_flux_skymap.yaxis) - 1):
                bin_ra = 0.5 * (
                    hist_flux_skymap.xaxis[bx] + hist_flux_skymap.xaxis[bx + 1]
                )
                bin_dec = 0.5 * (
                    hist_flux_skymap.yaxis[by] + hist_flux_skymap.yaxis[by + 1]
                )
                keep_event = False
                distance = pow(
                    pow(bin_ra - roi_x[0], 2) + pow(bin_dec - roi_y[0], 2), 0.5
                )
                if (
                ...
                if use_excl:
                    for roi in range(0, len(excl_roi_x)):
                        excl_distance = pow(
                            pow(bin_ra - excl_roi_x[roi], 2)
                            + pow(bin_dec - excl_roi_y[roi], 2),
                            0.5,
                        )
...
def GetRegionIntegral(
    ...
    excl_roi_y,
    excl_roi_r,
    hist_error_skymap=None,
    hist_syst_skymap=None,
    use_excl=True,
):
    ...
    for bx in range(0, len(hist_flux_skymap.xaxis) - 1):
        for by in range(0, len(hist_flux_skymap.yaxis) - 1):
            ...
            for roi in range(0, len(roi_x)):
                distance = pow(
                    pow(bin_ra - roi_x[roi], 2) + pow(bin_dec - roi_y[roi], 2), 0.5
                )
                if distance < roi_r[roi]:
                    keep_event = True
            if use_excl:
            ...
            if keep_event:
                ...
                if hist_syst_skymap == None:
                    ...
                    if np.isnan(hist_syst_skymap.waxis[bx, by, 0]):
                        continue
...
def GetRegionSpectrum(
    ...
    for binE in range(binE_start, binE_end):
        ...
        if hist_error_skymap != None and hist_syst_skymap != None:
            flux_sum, flux_stat_err = GetRegionIntegral(
                hist_flux_skymap[binE],
                roi_x,
                roi_y,
                roi_r,
                excl_roi_x,
                excl_roi_y,
                excl_roi_r,
                hist_error_skymap=hist_error_skymap[binE],
                hist_syst_skymap=hist_syst_skymap[binE],
                use_excl=use_excl,
            )
        ...
        x_axis += [
            0.5
            * (pow(10.0, logE_axis.xaxis[binE + 1]) + pow(10.0, logE_axis.xaxis[binE]))
        ]
        x_error += [
            0.5
            * (pow(10.0, logE_axis.xaxis[binE + 1]) - pow(10.0, logE_axis.xaxis[binE]))
...
def GetHessSS433e():
    ...
    for entry in range(0, len(energies)):
        energies[entry] = pow(10.0, energies[entry])
        fluxes[entry] = pow(10.0, fluxes[entry]) * erg_to_TeV
        flux_errs += [
            0.5
            * (pow(10.0, flux_errs_up[entry]) - pow(10.0, flux_errs_lo[entry]))
            * erg_to_TeV
        ]
...
def flux_lhaaso_wcda_j1908_func(x):
    ...
    Flux_N0 = 7.97
    Gamma_index = 2.42
    return Flux_N0 * pow(10, -13) * pow(x * 1.0 / 3000.0, -Gamma_index)
...
def GetHawcSaraFluxJ1908():
    ...
    for entry in range(0, len(energies)):
        ...
        flux_errs[entry] = (
            0.5
            * (flux_errs_up[entry] - flux_errs_low[entry])
            / (energies[entry] * energies[entry])
            * pow(energies[entry], 2)
        )
...
def GetFermiJordanFluxJ1908():
    ...
    fluxes = [
        2.856783157929038e-06,
        3.89109583469775e-06,
        5.0680678657082445e-06,
        9.271213817855382e-06,
    ]
    flux_stat_errs = [
        1.1604625384485099e-06,
        1.556189798998829e-06,
        2.2448723890895238e-06,
        3.4737117958614837e-06,
    ]
    flux_syst_errs = [
        1.135182978267407e-06,
        7.805371450450492e-07,
        1.6102184866176414e-06,
        1.5283362877401339e-06,
    ]
    flux_errs = []
    ...
    for entry in range(0, len(energies)):
        ...
        flux_errs += [
            pow(pow(flux_stat_errs[entry], 2) + pow(flux_syst_errs[entry], 2), 0.5)
        ]
...
def GetHAWCDiffusionFluxGeminga():
    ...
    energies = [pow(10.0, 0.90), pow(10.0, 1.60)]
    fluxes = [pow(10.0, -11.12), pow(10.0, -11.36)]
    flux_errs = [0.0, 0.0]
    flux_errs_up = [pow(10.0, -11.04), pow(10.0, -11.28)]
    flux_errs_low = [pow(10.0, -11.21), pow(10.0, -11.44)]
...
def GetHAWCGaussianFluxGeminga():
    ...
    energies = [pow(10.0, 0.90), pow(10.0, 1.60)]
    fluxes = [pow(10.0, -11.36), pow(10.0, -11.52)]
    flux_errs = [0.0, 0.0]
    flux_errs_up = [pow(10.0, -11.28), pow(10.0, -11.45)]
    flux_errs_low = [pow(10.0, -11.44), pow(10.0, -11.59)]
...
def GetHAWCDiskFluxGeminga():
    ...
    energies = [pow(10.0, 0.00), pow(10.0, 1.70)]
    fluxes = [pow(10.0, -11.42), pow(10.0, -11.81)]
    flux_errs = [0.0, 0.0]
    flux_errs_up = [pow(10.0, -11.30), pow(10.0, -11.68)]
    flux_errs_low = [pow(10.0, -11.56), pow(10.0, -11.94)]
...
def GetFermiFluxGeminga():
    ...
    energies = [pow(10.0, 1.03), pow(10.0, 1.30), pow(10.0, 1.56), pow(10.0, 1.82)]
    fluxes = [pow(10.0, -7.27), pow(10.0, -7.29), pow(10.0, -7.41), pow(10.0, -7.35)]
    flux_errs = [pow(10.0, -7.27), pow(10.0, -7.29), pow(10.0, -7.41), pow(10.0, -7.35)]
    flux_errs_up = [
        pow(10.0, -7.14),
        pow(10.0, -7.16),
        pow(10.0, -7.29),
    ...
    for entry in range(0, len(energies)):
        ...
        energies[entry] = energies[entry] / 1000.0
...
def GetFermiUpperLimitFluxGeminga():
    ...
    energies = [pow(10.0, 2.09), pow(10.0, 2.35), pow(10.0, 2.61), pow(10.0, 2.87)]
    fluxes = [pow(10.0, -7.35), pow(10.0, -7.23), pow(10.0, -7.34), pow(10.0, -7.18)]
    fluxes_err = [
        pow(10.0, -7.35),
        pow(10.0, -7.23),
        pow(10.0, -7.34),
...
def PrintAndPlotInformationRoI(
    fig,
    logE_min,
    logE_mid,
    logE_max,
    ...
    hist_syst_skymap,
    hist_flux_skymap,
    hist_flux_err_skymap,
    hist_flux_syst_skymap,
    hist_mimic_data_skymap,
    hist_mimic_bkgd_skymap,
    roi_name,
    roi_x,
    roi_y,
    roi_r,
    ...
    energy_axis, energy_error, flux, flux_incl_err = GetRegionSpectrum(
        hist_flux_skymap,
        roi_x,
        roi_y,
        roi_r,
        ...
        excl_roi_y,
        excl_roi_r,
        hist_error_skymap=hist_flux_err_skymap,
        hist_syst_skymap=hist_flux_syst_skymap,
    )
    energy_axis, energy_error, data, data_stat_err = GetRegionSpectrum(
        hist_data_skymap, roi_x, roi_y, roi_r, excl_roi_x, excl_roi_y, excl_roi_r
    )
    energy_axis, energy_error, bkgd, bkgd_syst_err = GetRegionSpectrum(
        hist_bkgd_skymap,
        roi_x,
        roi_y,
        roi_r,
        ...
        excl_roi_y,
        excl_roi_r,
        hist_syst_skymap=hist_syst_skymap,
    )
    ...
    for mimic in range(0, n_mimic):
        mimic_energy_axis, mimic_energy_error, mimic_data, mimic_data_stat_err = (
            GetRegionSpectrum(
                hist_mimic_data_skymap[mimic],
                roi_x,
                roi_y,
                roi_r,
                excl_roi_x,
        ...
        mimic_energy_axis, mimic_energy_error, mimic_bkgd, mimic_bkgd_stat_err = (
            GetRegionSpectrum(
                hist_mimic_bkgd_skymap[mimic],
                roi_x,
                roi_y,
                roi_r,
                excl_roi_x,
    ...
    print(
        f"RoI info: {roi_name[0]}, roi_x = {roi_x[0]}, roi_y = {roi_y[0]}, roi_r = {roi_r[0]}"
    )

    min_energy = pow(10.0, logE_bins[logE_min])
    mid_energy = pow(10.0, logE_bins[logE_mid])
    max_energy = pow(10.0, logE_bins[logE_max])
    ...
    print(
        f"E = {min_energy:0.2f}-{max_energy:0.2f} TeV, avg_E = {avg_energy:0.2f} TeV, data = {sum_data:0.1f}, bkgd = {sum_bkgd:0.1f} +/- {sum_error:0.1f}, significance = {significance:0.1f} sigma"
    )
    ...
    avg_energy = avg_energy / sum_flux
    print(
        f"E = {min_energy:0.2f}-{mid_energy:0.2f} TeV, avg_E = {avg_energy:0.2f} TeV, data = {sum_data:0.1f}, bkgd = {sum_bkgd:0.1f} +/- {sum_error:0.1f}, significance = {significance:0.1f} sigma"
    )
    ...
    avg_energy = avg_energy / sum_flux
    print(
        f"E = {mid_energy:0.2f}-{max_energy:0.2f} TeV, avg_E = {avg_energy:0.2f} TeV, data = {sum_data:0.1f}, bkgd = {sum_bkgd:0.1f} +/- {sum_error:0.1f}, significance = {significance:0.1f} sigma"
    )

    for binx in range(0, len(energy_axis)):
        significance = significance_li_and_ma(
            data[binx], bkgd[binx], bkgd_incl_err[binx]
        )
        print(
            f"E = {energy_axis[binx]:0.2f} ({pow(10.,logE_bins[binx]):0.2f}-{pow(10.,logE_bins[binx+1]):0.2f})TeV, data = {data[binx]:0.1f} +/- {data_stat_err[binx]:0.1f}, bkgd = {bkgd[binx]:0.1f} +/- {bkgd_incl_err[binx]:0.1f}, flux = {flux[binx]:0.2e} +/- {flux_incl_err[binx]:0.2e} TeV/cm2/s ({flux_cu[binx]:0.2f} CU), significance = {significance:0.1f} sigma"
        )
    print(
        "==============================================================================================================="
    )
    ...
    for x in range(0, len(flux)):
        if flux_err_cu[x] == 0.0:
            continue
        significance = flux_cu[x] / flux_err_cu[x]
        if significance < 2.0:
            cu_uplims[x] = 1.0
            flux_cu[x] = max(2.0 * flux_err_cu[x], flux_cu[x] + 2.0 * flux_err_cu[x])
    ...
    fig.savefig(
        f"output_plots/{source_name}_roi_flux_crab_unit_{roi_name[0]}.png",
        bbox_inches="tight",
    )
    axbig.remove()

    PrintSpectralDataForNaima(
        energy_axis, flux, flux_incl_err, f"VERITAS_{roi_name[0]}"
    )

    uplims = np.zeros_like(energy_axis)
    for x in range(0, len(flux)):
        if flux_incl_err[x] == 0.0:
            continue
        significance = flux[x] / flux_incl_err[x]
        if significance < 2.0:
            uplims[x] = 1.0
            flux[x] = max(2.0 * flux_incl_err[x], flux[x] + 2.0 * flux_incl_err[x])
    ...
    if "SS433" in source_name:
        ...
        HAWC_diff_energies, HAWC_diff_fluxes, HAWC_diff_flux_errs = (
            GetHAWCDiffusionFluxGeminga()
        )
        HAWC_disk_energies, HAWC_disk_fluxes, HAWC_disk_flux_errs = (
            GetHAWCDiskFluxGeminga()
        )
        HAWC_gaus_energies, HAWC_gaus_fluxes, HAWC_gaus_flux_errs = (
            GetHAWCGaussianFluxGeminga()
        )
        HESS_energies, HESS_fluxes, HESS_flux_errs = GetHessGeminga()
        Fermi_energies, Fermi_fluxes, Fermi_flux_errs = GetFermiFluxGeminga()
        Fermi_UL_energies, Fermi_UL_fluxes, Fermi_UL_err = (
            GetFermiUpperLimitFluxGeminga()
        )
        axbig.plot(HAWC_diff_energies, HAWC_diff_fluxes, "g-", label="HAWC diffusion")
        axbig.fill_between(
            HAWC_diff_energies,
            np.array(HAWC_diff_fluxes) - np.array(HAWC_diff_flux_errs),
            np.array(HAWC_diff_fluxes) + np.array(HAWC_diff_flux_errs),
            alpha=0.2,
            color="g",
        )
        ...
        axbig.fill_between(
            HAWC_disk_energies,
            np.array(HAWC_disk_fluxes) - np.array(HAWC_disk_flux_errs),
            np.array(HAWC_disk_fluxes) + np.array(HAWC_disk_flux_errs),
            alpha=0.2,
            color="m",
        )
        ...
        axbig.fill_between(
            HAWC_gaus_energies,
            np.array(HAWC_gaus_fluxes) - np.array(HAWC_gaus_flux_errs),
            np.array(HAWC_gaus_fluxes) + np.array(HAWC_gaus_flux_errs),
            alpha=0.2,
            color="y",
        )
    ...
    fig.savefig(
        f"output_plots/{source_name}_roi_energy_flux_{roi_name[0]}.png",
        bbox_inches="tight",
    )
    axbig.remove()
...
def DefineRegionOfInterest(src_name, src_ra, src_dec, coordinate_type="icrs"):
    ...
    if coordinate_type == "galactic":
        for roi in range(0, len(region_r)):
            region_x[roi], region_y[roi] = ConvertRaDecToGalactic(
                region_x[roi], region_y[roi]
            )
...
def SaveFITS(skymap_input, filename):
    ...
    reduced_wcs.wcs.crval = [
        central_ra,
        central_dec,
    ]  # Set the reference values for the physical coordinates
    ...
    fits.writeto(
        "output_plots/%s.fits" % (filename),
        new_image_data,
        reduced_wcs.to_header(),
        overwrite=True,
    )
...
def fit_2d_model(data_sky_map, bkgd_sky_map, src_x, src_y):
    ...
    for binx in range(0, nbins_x):
        for biny in range(0, nbins_y):
            image_excess[biny, binx] = (
                data_sky_map.waxis[binx, biny, 0] - bkgd_sky_map.waxis[binx, biny, 0]
            )
            image_error[biny, binx] = max(
                pow(data_sky_map.waxis[binx, biny, 0], 0.5), 1.0
            )
    ...
    lat = src_y
    sigma = 0.03807
    initial_prms += [(lon, lat, sigma, 10.0)]
    centroid_range = 0.5
    bound_lower_prms += [(lon - centroid_range, lat - centroid_range, sigma + 0.0, 0.0)]
    bound_upper_prms += [
        (lon + centroid_range, lat + centroid_range, sigma + 2.0, 1e10)
    ]
    ...
    popt, pcov = curve_fit(
        ...
        image_excess.ravel(),
        p0,
        sigma=image_error.ravel(),
        absolute_sigma=True,
        bounds=(p0_lower, p0_upper),
    )
...
def plot_radial_profile_with_systematics(
    fig,
    plotname,
    logE_min,
    logE_max,
    ...
    on_radial_axis, on_profile_axis, on_profile_err_axis = GetRadialProfile(
        flux_sky_map,
        flux_err_sky_map,
        roi_x,
        roi_y,
        ...
        excl_roi_y,
        excl_roi_r,
        radial_bin_scale=radial_bin_scale,
    )
    all_radial_axis, all_profile_axis, all_profile_err_axis = GetRadialProfile(
        flux_sky_map,
        flux_err_sky_map,
        roi_x,
        roi_y,
    ...
    for mimic in range(0, n_mimic):
        radial_axis, profile_axis, profile_err_axis = GetRadialProfile(
            mimic_flux_sky_map[mimic],
            mimic_flux_err_sky_map[mimic],
            roi_x,
            roi_y,
            2.0,
            excl_roi_x,
    ...
    baseline_yaxis = [0.0 for i in range(0, len(on_radial_axis))]
    axbig.plot(on_radial_axis, baseline_yaxis, color="b", ls="dashed")
    axbig.fill_between(
        on_radial_axis,
        -np.array(profile_syst_err_axis),
        np.array(profile_syst_err_axis),
        alpha=0.2,
        color="b",
        zorder=0,
    ...
    if fit_radial_profile:
        ...
        popt, pcov = curve_fit(
            ...
            curve_fit_brightness_array,
            p0=start,
            sigma=curve_fit_brightness_err_array,
            absolute_sigma=True,
        ...
        print(
            "diffusion radius = %0.2f +/- %0.2f deg (chi2/dof = %0.2f)"
            % (popt[1], pow(pcov[1][1], 0.5), chisq / dof)
        )
    ...
    lines += [
        "radial distance [deg] \t surface brightness [TeV/cm2/s/sr] \t brightness error [TeV/cm2/s/sr] \n"
    ]
    for entry in range(0, len(on_radial_axis)):
        print(
            f"x = {on_radial_axis[entry]:0.2f}, y = {on_profile_axis[entry]:0.2e}, err = {on_profile_err_axis[entry]:0.2e}"
        )
        lines += [
            f"{on_radial_axis[entry]:0.2f} \t {on_profile_axis[entry]:0.2e} \t {on_profile_err_axis[entry]:0.2e} \n"
        ]
    ...
    if fit_radial_profile:
        axbig.plot(
            on_radial_axis, diffusion_func(np.array(on_radial_axis), *popt), color="r"
        )
    fig.savefig(f"output_plots/{plotname}.png", bbox_inches="tight")
    axbig.remove()


def build_radial_symmetric_model(
    radial_symmetry_sky_map, on_radial_axis, on_profile_axis, roi_x, roi_y
):

    deg2_to_sr = 3.046 * 1e-4
    pix_size = (
        abs(
            (radial_symmetry_sky_map.yaxis[1] - radial_symmetry_sky_map.yaxis[0])
            * (radial_symmetry_sky_map.xaxis[1] - radial_symmetry_sky_map.xaxis[0])
        )
        * deg2_to_sr
    )
    ...
    for bx in range(0, len(radial_symmetry_sky_map.xaxis) - 1):
        for by in range(0, len(radial_symmetry_sky_map.yaxis) - 1):
            for br in range(0, len(on_radial_axis)):
                bin_ra = 0.5 * (
                    radial_symmetry_sky_map.xaxis[bx]
                    + radial_symmetry_sky_map.xaxis[bx + 1]
                )
                bin_dec = 0.5 * (
                    radial_symmetry_sky_map.yaxis[by]
                    + radial_symmetry_sky_map.yaxis[by + 1]
                )
                distance = pow(pow(bin_ra - roi_x, 2) + pow(bin_dec - roi_y, 2), 0.5)
                if distance < on_radial_axis[br] - 0.5 * radial_bin_size:
                    continue
                if distance >= on_radial_axis[br] + 0.5 * radial_bin_size:
                    continue
                if on_profile_axis[br] < 0.0:
                    continue
                radial_symmetry_sky_map.waxis[bx, by, 0] = (
                    on_profile_axis[br] * pix_size
                )
                break
...
def GetSlicedDataCubeMap(map_file, sky_map, vel_low, vel_up):
    ...
    map_center_lon, map_center_lat = ConvertRaDecToGalactic(
        map_center_ra, map_center_dec
    )
    ...
    image_data_reduced_z = np.full((image_data[:, :, vel_idx_start].shape), 0.0)
...
def GetSlicedDataCubeMapCGPS(map_file, sky_map, vel_low, vel_up):
    ...
    pixs_start = wcs.all_world2pix(
        map_center_lon, map_center_lat, vel_up * 1e3, 1.0, 1.0
    )
    pixs_end = wcs.all_world2pix(
        map_center_lon, map_center_lat, vel_low * 1e3, 1.0, 1.0
    )
    ...
    image_data_reduced_z = np.full((image_data[0, vel_idx_start, :, :].shape), 0.0)
    for idx in range(vel_idx_start, vel_idx_end):
    ...
    for idx_x in range(0, nbins_x):
        for idx_y in range(0, nbins_y):
            ...
            if pix_ra >= image_data_reduced_z.shape[0]:
                continue
            if pix_dec >= image_data_reduced_z.shape[1]:
                continue
            sky_map.waxis[idx_x, idx_y, 0] += image_data_reduced_z[pix_dec, pix_ra]
...
def GetSlicedDataCubeMapGALFA(map_file, sky_map, vel_low, vel_up):
    ...
    for idx_x in range(0, nbins_x):
        for idx_y in range(0, nbins_y):
            ...
            pix_ra = int(map_pixs[0])
            pix_dec = int(map_pixs[1])
            if pix_ra >= image_data_reduced_z.shape[1]:
                continue
            if pix_dec >= image_data_reduced_z.shape[0]:
                continue
            sky_map.waxis[idx_x, idx_y, 0] += image_data_reduced_z[pix_dec, pix_ra]
...
def compute_camera_frame_power_spectrum(skymap, idx_z=0):
    ...
    v_power_spectrum = [0.0] * magnitude.shape[0]
    h_power_spectrum = [0.0] * magnitude.shape[1]
    v_power_spectrum_noise = [0.0] * magnitude.shape[0]
    h_power_spectrum_noise = [0.0] * magnitude.shape[1]
    ...
    for idx_x in range(0, len(v_power_spectrum_noise_shifted)):
        if v_power_spectrum_noise_shifted[idx_x] == 0.0:
            v_power_spectrum_shifted[idx_x] = 0.0
        else:
            v_power_spectrum_shifted[idx_x] = (
                v_power_spectrum_shifted[idx_x] - v_power_spectrum_noise_shifted[idx_x]
            ) / v_power_spectrum_noise_shifted[idx_x]
    for idx_y in range(0, len(h_power_spectrum_noise_shifted)):
        if h_power_spectrum_noise_shifted[idx_y] == 0.0:
            h_power_spectrum_shifted[idx_y] = 0.0
        else:
            h_power_spectrum_shifted[idx_y] = (
                h_power_spectrum_shifted[idx_y] - h_power_spectrum_noise_shifted[idx_y]
            ) / h_power_spectrum_noise_shifted[idx_y]
...
def plot_camera_frame_power_spectrum(fig, plotname, skymap, idx_z=0):
    ...
    freqs_shifted, v_power_spectrum, h_power_spectrum = (
        compute_camera_frame_power_spectrum(skymap, idx_z=idx_z)
    )
    fig.clf()
    figsize_x = 6.4
    figsize_y = 4.6
...
def build_skymap(
    ...
    if "MIMIC" in onoff:
        ...
        for run in range(0, len(runlist)):
            if run >= len(mimic_runlist):
                continue
            if mimic_index >= len(mimic_runlist[run]):
                continue
    ...
    data_xyoff_map_1d_monospec = convert_multivar_to_xyoff_vector1d_monospec(
        data_multivar_map_1d_fullspec
    )
    mask_xyoff_map_1d_monospec = convert_multivar_to_xyoff_vector1d_monospec(
        mask_multivar_map_1d_fullspec
    )
    data_xyoff_map_1d_fullspec = convert_multivar_to_xyoff_vector1d(
        data_multivar_map_1d_fullspec
    )
    mask_xyoff_map_1d_fullspec = convert_multivar_to_xyoff_vector1d(
        mask_multivar_map_1d_fullspec
    )
    data_xyvar_map_1d_fullspec = convert_multivar_to_xyvar_vector1d(
        data_multivar_map_1d_fullspec
    )
    mask_xyvar_map_1d_fullspec = convert_multivar_to_xyvar_vector1d(
        mask_multivar_map_1d_fullspec
    )

    if use_offruns:
        off_data_multivar_map_1d_fullspec = np.zeros_like(big_off_matrix_fullspec[0])
        off_mask_multivar_map_1d_fullspec = np.zeros_like(
            big_off_mask_matrix_fullspec[0]
        )
        for entry in range(0, len(big_off_matrix_fullspec)):
            off_data_multivar_map_1d_fullspec += np.array(
                big_off_matrix_fullspec[entry]
            )
            off_mask_multivar_map_1d_fullspec += np.array(
                big_off_mask_matrix_fullspec[entry]
            )
        off_data_xyoff_map_1d_fullspec = convert_multivar_to_xyoff_vector1d(
            off_data_multivar_map_1d_fullspec
        )
        off_mask_xyoff_map_1d_fullspec = convert_multivar_to_xyoff_vector1d(
            off_mask_multivar_map_1d_fullspec
        )
        off_data_xyvar_map_1d_fullspec = convert_multivar_to_xyvar_vector1d(
            off_data_multivar_map_1d_fullspec
        )
        off_mask_xyvar_map_1d_fullspec = convert_multivar_to_xyvar_vector1d(
            off_mask_multivar_map_1d_fullspec
        )
    ...
    for logE in range(0, logE_nbins):
        for gcut in range(0, gcut_bins):
            for idx_x in range(0, xoff_bins[logE]):
                for idx_y in range(0, yoff_bins[logE]):
                    idx_1d = xyoff_idx_1d[gcut][logE][idx_x][idx_y]
                    data_xyoff_map[logE].waxis[idx_x, idx_y, gcut] = (
                        data_xyoff_map_1d_fullspec[idx_1d]
                    )
                    if use_offruns:
                        init_xyoff_map[logE].waxis[idx_x, idx_y, gcut] = (
                            off_data_xyoff_map_1d_fullspec[idx_1d]
                        )
                        fit_xyoff_map[logE].waxis[idx_x, idx_y, gcut] = (
                            off_data_xyoff_map_1d_fullspec[idx_1d]
                        )
                    else:
                        init_xyoff_map[logE].waxis[idx_x, idx_y, gcut] = (
                            avg_xyoff_map_1d_fullspec[idx_1d]
                        )
                        fit_xyoff_map[logE].waxis[idx_x, idx_y, gcut] = (
                            avg_xyoff_map_1d_fullspec[idx_1d]
                        )
    ...
    for logE in range(0, logE_nbins):
        for idx_x in range(0, xvar_bins[logE]):
            for idx_y in range(0, yvar_bins[logE]):
                ...
                data_xyvar_map[logE].waxis[idx_x, idx_y, 0] = (
                    data_xyvar_map_1d_fullspec[idx_1d]
                )

    sr_map_1d_truth, cr_map_1d = prepare_vector_for_least_square(
        data_multivar_map_1d_fullspec
    )
    ls_model = nn_model[0]
    ...
    for logE in range(0, logE_nbins):
        norm_cr_data = np.sum(data_xyoff_map[logE].waxis[:, :, :]) - np.sum(
            data_xyoff_map[logE].waxis[:, :, 0]
        )
        norm_sr_data = sr_map_1d[logE]
        norm_sr_data_err = sr_map_1d_err[logE]
        norm_sr_init = np.sum(init_xyoff_map[logE].waxis[:, :, 0])
        norm_cr_init = np.sum(init_xyoff_map[logE].waxis[:, :, :]) - np.sum(
            init_xyoff_map[logE].waxis[:, :, 0]
        )
    ...
    if not use_init:
        ...
        for logE in range(0, logE_nbins):
            ...
            print(
                f"truth = {truth:0.1f}, prediction = {prediction:0.1f}+/-{error:0.1f}"
            )

        xyoff_effective_matrix_rank_fullspec = big_xyoff_eigenvectors_fullspec.shape[0]

        xyoff_truth_params = (
            big_xyoff_eigenvectors_fullspec @ data_xyoff_map_1d_fullspec
        )
        ...
        for entry in range(0, len(fit_params)):
            print(
                f"init_params = {init_params[entry]:0.1f}, truth_params = {xyoff_truth_params[entry]:0.1f}, fit_params = {fit_params[entry]:0.1f}"
            )
    ...
    for logE in range(0, logE_nbins):
        ...
        print(
            f"sum_data_xyoff_map = {sum_data_xyoff_map:0.1f}, sum_fit_xyoff_map = {sum_fit_xyoff_map:0.1f}"
        )

    syst_scaling = []
    for logE in range(0, logE_nbins):
        sum_xyoff_map_sr = 0.0
        sum_xyoff_map_sr = np.sum(fit_xyoff_map[logE].waxis[:, :, 0])
        if np.sum(fit_xyoff_map[logE].waxis[:, :, 1]) > 0.0:
            syst_scaling += [
                np.sum(fit_xyoff_map[logE].waxis[:, :, 0])
                / np.sum(fit_xyoff_map[logE].waxis[:, :, 1])
            ]
        else:
            syst_scaling += [0.0]
        for idx_x in range(0, xoff_bins[logE]):
    ...
    for run in range(0, len(runlist)):
        ...
        exposure_hours += (time_end - time_start) / 3600.0
        avg_tel_elev += TelElevation * (time_end - time_start) / 3600.0
        avg_tel_azim += TelAzimuth * (time_end - time_start) / 3600.0
        for entry in range(0, total_entries):
            EvtTree.GetEntry(entry)
            RA = EvtTree.RA
            DEC = EvtTree.DEC
            ...
            if NImages < 2:
                continue
            if not ApplyTimeCuts(Time - time_start, list_timecuts):
                continue
            if logE < 0:
                continue
            if logE >= len(data_sky_map):
                continue
            if Energy < min_Energy_cut:
                continue
            if Energy > max_Energy_cut:
            ...
            if onoff == "OFF":
                found_gamma_source = CoincideWithBrightStars(
                    Xsky, Ysky, gamma_source_coord
                )
                if found_gamma_source:
                    continue
            if "MIMIC" in onoff:
                found_gamma_source = CoincideWithBrightStars(
                    RA, DEC, gamma_source_coord
                )
                if found_gamma_source:
                    continue
            ...
            if onoff == "OFF":
                ...
                if GammaCut > 1.0:
                    if GammaCut < 2.0:
                        ...
                        syst_sky_map[logE].fill(
                            Xsky_rel, Ysky_rel, 0.5, weight=syst_scaling[logE]
                        )
                    elif GammaCut < 3.0:
                        syst_sky_map[logE].fill(
                            Xsky_rel,
                            Ysky_rel,
                            0.5,
                            weight=-1.0 * sr_syst * syst_scaling[logE],
                        )
                elif GammaCut < 1.0:
                ...
                if coordinate_type == "galactic":
                    ...
                    if GammaCut > 1.0:
                        if GammaCut < 2.0:
                            fit_sky_map[logE].fill(
                                Gal_Xsky, Gal_Ysky, 0.5, weight=sr_model
                            )
                            syst_sky_map[logE].fill(
                                Gal_Xsky, Gal_Ysky, 0.5, weight=syst_scaling[logE]
                            )
                        elif GammaCut < 3.0:
                            syst_sky_map[logE].fill(
                                Gal_Xsky,
                    ...
                    if GammaCut > 1.0:
                        if GammaCut < 2.0:
                            ...
                            syst_sky_map[logE].fill(
                                Xsky, Ysky, 0.5, weight=syst_scaling[logE]
                            )
                        elif GammaCut < 3.0:
                            syst_sky_map[logE].fill(
                                Xsky,
