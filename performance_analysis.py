    ...
    for epoch in input_epoch:
        for src in input_sources:
            ...
            for run in range(0, len(analysis_result)):
                ...
                if (
                    current_exposure > exposure_per_group
                    or run == len(analysis_result) - 1
                ):
    ...
    for grp in range(0, len(analysis_data[ana])):
        ...
        for demoE in range(0, demoE_nbins):
            freqs_shifted, v_power_spectrum, h_power_spectrum = (
                compute_camera_frame_power_spectrum(grp_diff_map[demoE])
            )
...
for demoE in range(0, demoE_nbins):
    ...
    for ana in range(0, len(ana_tag)):
        ...
        for grp in range(0, len(list_elev[ana])):
            ...
            plot_elev += [elev]
            plot_error_significance += [abs(significance_li_and_ma(data, bkgd, 0.0))]
        ax.scatter(
            plot_elev, plot_error_significance, alpha=0.3, label=f"{ana_tag[ana][1]}"
        )
        # ax.scatter(plot_elev,plot_error_significance,alpha=0.3, label=f"{ana_tag[ana][0].strip('rank')} eigenvectors")
    ax.legend(loc="best")
    fig.savefig(
        f"output_plots/fov_error_significance_vs_elev_demoE{demoE}.png",
        bbox_inches="tight",
    )
    del fig
    del ax
...
for demoE in range(0, demoE_nbins):
    ...
    for ana in range(0, len(ana_tag)):
        ...
        for grp in range(0, len(list_nsb[ana])):
            ...
            plot_error_significance += [abs(significance_li_and_ma(data, bkgd, 0.0))]
        ax.scatter(
            plot_nsb, plot_error_significance, alpha=0.3, label=f"{ana_tag[ana][1]}"
        )
        # ax.scatter(plot_nsb,plot_error_significance,alpha=0.3, label=f"{ana_tag[ana][0].strip('rank')} eigenvectors")
    ax.legend(loc="best")
    fig.savefig(
        f"output_plots/fov_error_significance_vs_nsb_demoE{demoE}.png",
        bbox_inches="tight",
    )
    del fig
    del ax
...
for demoE in range(0, demoE_nbins):
    ...
    for ana in range(0, len(ana_tag)):
        ...
        for b in range(0, n_bins):
            ...
            for entry in range(0, len(plot_elev)):
                if plot_elev[entry] < elev_low:
                    continue
                if plot_elev[entry] > elev_up:
                    continue
    ...
    ax.legend(loc="best")
    E_min = pow(10.0, demo_energy[demoE])
    E_max = pow(10.0, demo_energy[demoE + 1])
    ax.set_title(f"E = {E_min:0.2f} - {E_max:0.2f} TeV")
    fig.savefig(
        f"output_plots/pix_error_significance_vs_elev_demoE{demoE}.png",
        bbox_inches="tight",
    )
    del fig
    del ax
...
for demoE in range(0, demoE_nbins):
    ...
    for ana in range(0, len(ana_tag)):
        ...
        for b in range(0, n_bins):
            ...
            for entry in range(0, len(plot_nsb)):
                if plot_nsb[entry] < nsb_low:
                    continue
                if plot_nsb[entry] > nsb_up:
                    continue
                avg_significance += plot_error_significance[entry]
                n_entries += 1.0
            if n_entries > 0.0:
    ...
    ax.set_title(f"E = {E_min:0.2f} - {E_max:0.2f} TeV")
    fig.savefig(
        f"output_plots/pix_error_significance_vs_nsb_demoE{demoE}.png",
        bbox_inches="tight",
    )
    del fig
    del ax
...
for demoE in range(0, demoE_nbins):
    ...
    for ana in range(0, len(ana_tag)):
        ...
        hist_significance, bin_edges = np.histogram(
            plot_error_significance, bins=hist_bins, range=(-hist_range, hist_range)
        )
        ...
        ax.hist(
            ...
            histtype="step",
            density=True,
            label=f"{ana_tag[ana][1]}",
        )

    hist_binsize = 2.0 * hist_range / float(hist_bins)
    significance_axis = np.arange(-5.0, 5.0, 0.01)
    total_entries = 1.0
    normal_dist = (
        hist_binsize
        * total_entries
        * np.exp(-significance_axis * significance_axis / 2.0)
    )
    ...
    ax.set_title(f"E = {E_min:0.2f} - {E_max:0.2f} TeV")
    fig.savefig(
        f"output_plots/error_significance_distribution_demoE{demoE}.png",
        bbox_inches="tight",
    )
    del fig
    del ax
...
for src in range(0, len(src_keys)):
    ...
    for demoE in range(0, demoE_nbins):
        for ana in range(0, len(ana_tag)):
            ...
            data_dict = dict.fromkeys(src_keys, 0.0)  # Initializes all values to 0.
            bkgd_dict = dict.fromkeys(src_keys, 0.0)  # Initializes all values to 0.
            ...
            if data == 0.0:
                ...
                syst = (data - bkgd) / data
                stat = 1.0 / pow(data, 0.5)
            print(
                f"{src_name}, {expo_dict[src_name]:0.1f} hrs, syst error = {syst*100.:0.2f} +/- {stat*100.:0.2f} %"
            )
...
for demoE in range(0, demoE_nbins):
    ...
    for ana in range(0, len(ana_tag)):
        ...
        syst_err_axis += [syst_err * 100.0]
        stat_err_axis += [
            pow(stat_err * stat_err + syst_err * syst_err / n_groups, 0.5) * 100.0
        ]
        print(
            f"E = {pow(10.,demo_energy[demoE]):0.3f} TeV, {ana_tag[ana][0]}, error = {syst_err*100.:0.2f} +/- {stat_err*100.:0.2f} +/- {syst_err/pow(n_groups,0.5)*100.:0.2f} % ({syst_err/stat_err:0.2f} sigma)"
        )
    ...
    E_max = pow(10.0, demo_energy[demoE + 1])
    ax.errorbar(
        ana_axis, syst_err_axis, yerr=stat_err_axis, color="k", marker=".", ls="solid"
    )
