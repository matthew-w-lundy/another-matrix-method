...
output_filename = (
    f"{smi_output}/skymaps_{source_name}_{input_epoch}_{onoff}_{sky_tag}.pkl"
)
...
on_runlist, off_runlist, mimic_runlist = ReadRunListFromFile(
    smi_input, on_file, off_file, mimic_file
)
...
for run in range(0, total_runs):
    ...
    total_exposure += (time_end - time_start) / 3600.0
...
for run in range(0, total_runs):
    ...
    small_off_runlist += off_runlist[run]
    small_mimic_runlist += [mimic_runlist[run]]
    run_exposure += (time_end - time_start) / 3600.0
...
big_runlist, big_off_runlist, big_mimic_runlist = shuffle_three_lists(
    big_runlist, big_off_runlist, big_mimic_runlist
)
...
for small_runlist in range(0, len(big_runlist)):
    ...
    run_azim = run_info[2]
    run_nsb = run_info[3]
    if run_exposure_hours == 0.0:
        continue
    print(f"run_exposure_hours = {run_exposure_hours}")
    print(f"run_elev = {run_elev}")
    print(f"run_nsb = {run_nsb}")

    run_data_xyoff_sum_sr = 0.0
    run_data_sky_sum_sr = 0.0
    run_fit_xyoff_sum_sr = 0.0
    run_fit_sky_sum_sr = 0.0
    for logE in range(0, logE_nbins):
    ...
    print(
        f"run_data_xyoff_sum_sr = {run_data_xyoff_sum_sr:0.1f}, run_data_sky_sum_sr = {run_data_sky_sum_sr:0.1f}"
    )
    print(
        f"run_fit_xyoff_sum_sr = {run_fit_xyoff_sum_sr:0.1f}, run_fit_sky_sum_sr = {run_fit_sky_sum_sr:0.1f}"
    )
    ...
    for logE in range(0, logE_nbins):
        ...
        if data_sum > 0.0:
            ...
            syst_error = 100.0 * (syst_sum) / data_sum
        print(
            f"On data,  data_sum = {data_sum}, bkgd_sum = {bkgd_sum:0.1f}, error = {error:0.1f} +/- {stat_error:0.1f} +/- {syst_error:0.1f} %"
        )
    print(
        f"total_data_sum = {total_data_sum:0.1f}, total_bkgd_sum = {total_bkgd_sum:0.1f}"
    )
    ...
    if not os.path.exists(output_filename):
        ...
        analysis_result += [
            [
                [run_exposure_hours, run_elev, run_azim, run_nsb],
                incl_sky_map,
                data_sky_map,
                bkgd_sky_map,
                syst_sky_map,
