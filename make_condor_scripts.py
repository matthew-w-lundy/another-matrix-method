...
for s in range(0, len(input_params)):
    ...
    if onoff == "ON" and (not "Crab" in source):
        for mimic in range(1, n_mimic + 1):
            file.write(
                f'python3 build_eigenvectors.py "{source}" "MIMIC{mimic}" "V6"\n'
            )
            file.write(
                f'python3 build_eigenvectors.py "{source}" "MIMIC{mimic}" "V5"\n'
            )
            file.write(
                f'python3 build_eigenvectors.py "{source}" "MIMIC{mimic}" "V4"\n'
            )
    file.close()
...
for s in range(0, len(input_params)):
    ...
    file.write(
        f'python3 save_skymaps.py "{source}" {src_ra} {src_dec} "{onoff}" "V6"\n'
    )
    file.write(
        f'python3 save_skymaps.py "{source}" {src_ra} {src_dec} "{onoff}" "V5"\n'
    )
    file.write(
        f'python3 save_skymaps.py "{source}" {src_ra} {src_dec} "{onoff}" "V4"\n'
    )
    if onoff == "ON" and (not "Crab" in source):
        for mimic in range(1, n_mimic + 1):
            file.write(
                f'python3 save_skymaps.py "{source}" {src_ra} {src_dec} "MIMIC{mimic}" "V6"\n'
            )
            file.write(
                f'python3 save_skymaps.py "{source}" {src_ra} {src_dec} "MIMIC{mimic}" "V5"\n'
            )
            file.write(
                f'python3 save_skymaps.py "{source}" {src_ra} {src_dec} "MIMIC{mimic}" "V4"\n'
            )
    file.close()
...
for s in range(0, len(input_params)):
    ...
    file.write(
        f'python3 plot_analysis_result.py "{source}" {src_ra} {src_dec} "{onoff}" \n'
    )
    file.close()
