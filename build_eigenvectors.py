...
convert_multivar_to_xyoff_vector1d_monospec = (
    common_functions.convert_multivar_to_xyoff_vector1d_monospec
)
...
result = subprocess.run(
    ["ls", f"{smi_output}/{ana_dir}"], capture_output=True, text=True
)
...
for k in range(0, len(file_list)):
    if (
        f"big_off_matrix_{source_name}_{onoff}_{input_epoch}_{cr_tag}_{bin_tag}"
        in file_list[k]
    ):
...
for entry in range(0, len(big_matrix_fullspec)):
    ...
    if expo < 15.0 / 60.0 or norm < 200.0:
...
for entry in range(0, len(big_matrix_fullspec)):
    if entry in delete_entries:
        continue
    norm = big_exposure[entry]
    new_xyoff_matrix_monospec += [
        convert_multivar_to_xyoff_vector1d_monospec(big_matrix_fullspec[entry] / norm)
    ]
    new_xyoff_matrix_fullspec += [
        convert_multivar_to_xyoff_vector1d(big_matrix_fullspec[entry] / norm)
    ]
    new_xyvar_matrix_fullspec += [
        convert_multivar_to_xyvar_vector1d(big_matrix_fullspec[entry] / norm)
    ]
...
for entry in range(0, len(big_matrix_fullspec)):
    if entry in delete_entries:
        continue
    norm = big_exposure[entry]
    sr_map_1d, cr_map_1d = prepare_vector_for_least_square(
        big_matrix_fullspec[entry] / norm
    )
...
model_err = []
for logE in range(0, logE_nbins):
    logE_model, logE_model_err = weighted_least_square_solution(
        list_cr_map_1d[logE],
        list_sr_map_1d[logE],
        list_sr_weight[logE],
        plot_tag=f"{source_name}_{onoff}_{input_epoch}_{sky_tag}",
    )
    model += [logE_model]
...
for logE in range(0, logE_nbins):
    for gcut in range(0, gcut_bins):
        ...
        im = axbig.imshow(
            avg_xyoff_map[logE].waxis[:, :, gcut].T,
...
fig.savefig(
    f"output_plots/fullspec_avgmap_xyoff_{source_name}_{input_epoch}_tanspose",
    bbox_inches="tight",
)
axbig.remove()
...
for logE in range(0, logE_nbins):
    U_full, S_full, VT_full = np.linalg.svd(
        new_xyoff_matrix_monospec[:, logE, :], full_matrices=False
    )  # perform better for perturbation method
    effective_matrix_rank_monospec = min(
        matrix_rank_fullspec, int(0.5 * (len(S_full) - 1))
    )
...
U_full, S_full, VT_full = np.linalg.svd(
    new_xyoff_matrix_fullspec, full_matrices=False
)  # perform better for perturbation method
...
axbig.set_ylim(S_full[plot_max_rank - 1], 2.0 * S_full[0])
axbig.set_xscale("log")
axbig.set_yscale("log")
axbig.tick_params(axis="both", which="major", labelsize=fig_fontsize)
axbig.plot(rank_index, S_full)
fig.savefig(
    f"{smi_dir}/output_plots/xyoff_singularvalue_{source_name}_{input_epoch}.png",
    bbox_inches="tight",
)
axbig.remove()
...
for rank in range(0, max_matrix_rank):
    ...
    eigen_xyoff_map = convert_xyoff_vector1d_to_map3d(
        big_xyoff_eigenvectors_fullspec[rank] * (-1.0)
    )
    ...
    for logE in range(0, logE_nbins):
        for gcut in range(0, gcut_bins):
            ...
            zmax = max(abs(wmax), abs(wmin))
            im = axbig.imshow(
                eigen_xyoff_map[logE].waxis[:, :, gcut].T,
                extent=(xmin, xmax, ymin, ymax),
                vmin=-zmax,
                vmax=zmax,
                aspect="auto",
    ...
    fig.savefig(
        f"output_plots/fullspec_eigenmap_xyoff_{source_name}_{input_epoch}_rank{rank}_tanspose",
        bbox_inches="tight",
    )
    axbig.remove()
...
with open(output_filename, "wb") as file:
    ...
    models += [
        [
            big_xyoff_eigenvalues_fullspec,
            big_xyoff_eigenvectors_fullspec,
            avg_xyoff_map_1d_fullspec,
        ]
    ]
    models += [
        [
