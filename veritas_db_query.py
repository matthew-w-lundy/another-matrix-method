...
input_range_elev = 10.0
input_range_azim = 0.20
input_range_nsb = 2.0
input_range_runnum = 20000.0
find_imposter = False
...
def print_all_runs_nsb():
    ...
    with open("/nevis/ged/data/rshang/SMI_AUX/NSB_allruns.txt", "w") as file:
        ...
        for x in res_runtime:
            ...
            if x["run_id"] == previous_run_id:
                continue
            previous_run_id = x["run_id"]

            # if x['run_id']<46642: continue
            if all_runs_weather[x["run_id"]] == None:
                continue
            if "C" in all_runs_weather[x["run_id"]]:
                continue
            if "D" in all_runs_weather[x["run_id"]]:
                continue
            if "F" in all_runs_weather[x["run_id"]]:
                continue

            if all_runs_src[x["run_id"]] == None:
                continue
            if all_runs_src[x["run_id"]] == "engineering":
                continue
            if all_runs_src[x["run_id"]] == "other":
                continue
            if all_runs_src[x["run_id"]] == "none":
                continue
            if all_runs_src[x["run_id"]] == "qi":
                continue
            if all_runs_src[x["run_id"]] == "NOSOURCE":
                continue

            if all_runs_type[x["run_id"]] == "flasher":
                continue
            ...
            for tel in range(0, 1):
                for ch in range(1, 10):
                    ...
                    total_entries = 0.0
                    query = (
                        "SELECT current_meas FROM tblHV_Telescope%s_Status WHERE db_start_time>'%s' AND db_start_time<'%s' AND channel=%s"
                        % (tel, run_start_time, run_end_time, ch * 40)
                    )
                    crs.execute(query)
                    res2 = crs.fetchall()
                    for y in res2:
...
def get_run_nsb(run_id):
    ...
    query = (
        "SELECT run_id,db_start_time,db_end_time FROM tblProcess_RunTime WHERE run_id=%s"
        % (run_id)
    )
    crs.execute(query)
    res = crs.fetchall()
    ...
    for ch in range(1, 500):
        ...
        total_entries = 0.0
        query = (
            "SELECT current_meas FROM tblHV_Telescope1_Status WHERE db_start_time>'%s' AND db_start_time<'%s' AND channel=%s"
            % (run_start_time, run_end_time, ch)
        )
        crs.execute(query)
        res = crs.fetchall()
        for x in res:
...
def get_all_runs_info(epoch, obs_type):
    ...
    for x in res_run_info:
        ...
        if x["source_id"] == None:
            continue
        if x["source_id"] == "engineering":
            continue
        if x["source_id"] == "other":
            continue
        if x["source_id"] == "none":
            continue
        if x["source_id"] == "qi":
            continue
        if x["source_id"] == "NOSOURCE":
            continue

        if x["run_id"] in all_runs_comments:
            run_usable_time = all_runs_comments[x["run_id"]]
            if run_usable_time < 5.0 * 60.0:
                continue
        else:
            continue
    ...
    with open("/nevis/ged/data/rshang/SMI_AUX/l3rate_allruns.txt", "w") as file:
        ...
        for x in res_run_info:
            ...
            if timestamp_end == "None":
                continue
            timestamp_start = (
                timestamp_start.replace(" ", "").replace("-", "").replace(":", "")
            )
            timestamp_end = (
                timestamp_end.replace(" ", "").replace("-", "").replace(":", "")
            )
            timestamp_start += "000"
            timestamp_end += "000"
            l3_avg_run = 0.0
            total_entries = 0.0
            query = (
                "SELECT L3 FROM tblL3_Array_TriggerInfo WHERE timestamp>%s AND timestamp<%s AND run_id=%s"
                % (timestamp_start, timestamp_end, x["run_id"])
            )
            crs.execute(query)
...
def print_all_runs_el_az():
    ...
    with open("/nevis/ged/data/rshang/SMI_AUX/elaz_allruns.txt", "w") as file:
        ...
        for x in res_run_info:
            ...
            total_entries = 0.0
            query = (
                "SELECT elevation_target,azimuth_target FROM tblPositioner_Telescope1_Status WHERE timestamp>%s AND timestamp<%s"
            ...
            if total_entries == 0.0:
                continue
            el_avg_run = el_avg_run / total_entries * 180.0 / math.pi
            az_avg_run = az_avg_run / total_entries * 180.0 / math.pi
...
def get_run_el_az(run_id):
    ...
    query = (
        "SELECT run_id,data_start_time,data_end_time FROM tblRun_Info WHERE run_id=%s"
        % (run_id)
    )
    crs.execute(query)
    res = crs.fetchall()
...
def get_run_category(run_id):
    ...
    query = (
        "SELECT run_id,data_category FROM tblRun_Analysis_Comments WHERE run_id=%s"
        % (run_id)
    )
    crs.execute(query)
    # fetch from cursor
...
def get_run_timecut(run_id):
    ...
    query = (
        "SELECT run_id,time_cut_mask FROM tblRun_Analysis_Comments WHERE run_id=%s"
        % (run_id)
    )
    crs.execute(query)
    # fetch from cursor
    res = crs.fetchall()
    print(
        "run_id = %s, time_cut_mask = %s" % (res[0]["run_id"], res[0]["time_cut_mask"])
    )
...
def get_run_ra_dec(run_id):
    ...
    query = (
        "SELECT run_id,source_id,offsetRA,offsetDEC,offset_angle FROM tblRun_Info WHERE run_id=%s"
        % (run_id)
    )
    crs.execute(query)
    # fetch from cursor
    ...
    query2 = (
        "SELECT source_id,ra,decl FROM tblObserving_Sources WHERE source_id='%s'"
        % (source_name)
    )
    crs.execute(query2)
    res2 = crs.fetchall()
    ...
    run_ra = (source_ra + res[0]["offsetRA"]) * 180.0 / math.pi
    run_dec = (source_dec + res[0]["offsetDEC"]) * 180.0 / math.pi
...
def find_runs_near_galactic_plane(
    obs_name, epoch, obs_type, gal_b_low, gal_b_up, nsb_low, nsb_up, elev_range
):
    ...
    for x in res:
        ...
        source_ra = x["ra"] * 180.0 / math.pi
        source_dec = x["decl"] * 180.0 / math.pi
    ...
    for x in res:
        ...
        if "PSR_J2032_p4127_baseline" in obs_name:
            if x["run_id"] >= 86880 and x["run_id"] <= 88479:
                continue
        ...
        if x["run_id"] in all_runs_type:
            ...
            if run_type != obs_type:
                print(
                    "RUN %s rejected because of observation type = %s"
                    % (x["run_id"], run_type)
                )
                continue
        else:
        ...
        if x["run_id"] in all_runs_weather:
            ...
            if run_weather == None:
                print(
                    "RUN %s rejected because of weather = %s"
                    % (x["run_id"], run_weather)
                )
                continue
            if "C" in run_weather:
                print(
        ...
        if x["run_id"] in all_runs_duration:
            ...
            if run_duration < 5.0 * 60.0:
                print(
                    "RUN %s rejected because of duration = %s"
                    % (x["run_id"], run_duration)
                )
                continue
        else:
        ...
        if on_run_el < elev_range[0]:
            print(
                "RUN %s rejected because of elevation = %s" % (x["run_id"], on_run_el)
            )
            continue
...
def find_on_runs_around_source(
    obs_name, obs_ra, obs_dec, epoch, obs_type, elev_range, search_radius
):
    ...
    for x in res:
        ...
        if "CrabNebula" in obs_name:
            if "1p0wobble" in obs_name:
                if run_offset > 1.2 or run_offset < 0.6:
                    continue
            elif "1p5wobble" in obs_name:
                if run_offset > 1.8 or run_offset < 1.2:
                    continue
            else:
                if run_offset > 0.6 or run_offset < 0.0:
                    continue
...
def find_off_runs_around_source(
    obs_name,
    obs_ra,
    obs_dec,
    epoch,
    ...
    for x in res:
        ...
        distance = pow(pow(obs_ra - source_ra, 2) + pow(obs_dec - source_dec, 2), 0.5)
        if distance > 10.0:
            if "HWC" in source_name:
                continue
            if abs(source_ra - 83.633) < 2.0 and abs(source_dec - 22.014) < 2.0:
                continue  # Crab
            if abs(source_ra - 166.079) < 2.0 and abs(source_dec - 38.195) < 2.0:
                continue  # Mrk 421
            if abs(source_ra - 253.467) < 2.0 and abs(source_dec - 39.76) < 2.0:
                continue  # Mrk 501
            if abs(source_ra - 98.117) < 3.0 and abs(source_dec - 17.367) < 3.0:
                continue  # Geminga
            if abs(source_gal_b) > 10.0:
    ...
    for on_run in range(0, len(list_on_run_ids)):
        ...
        for run in range(0, len(all_runs_info)):
            ...
            if number_off_runs >= require_nmatch:
                continue

            if all_runs_info[run][0] == list_on_run_ids[on_run]:
                continue
            ...
            if file_name == "ImposterList":
                ...
                for off_run in range(0, len(list_off_run_ids)):
                    if all_runs_info[run][0] == list_off_run_ids[off_run][1]:
                        already_used = True
                if already_used:
                    continue
            ...
            delta_azim = math.cos(off_run_az * math.pi / 180.0) - math.cos(
                on_run_az * math.pi / 180.0
            )
            ...
            sorted_dict_desc = dict(
                sorted(param_dict.items(), key=lambda item: item[1], reverse=True)
            )
            first_key = next(iter(param_dict))
            first_value = param_dict[first_key]

            if abs(delta_elev) > 1.0 * range_elev:
                continue
            if abs(delta_nsb) > 1.0 * range_nsb:
                continue
            if abs(delta_runnum) > 1.0 * range_runnum:
                continue
            if abs(delta_azim) > 1.0 * range_azim:
                continue
            ...
            list_off_run_ids += [
                [
                    int(list_on_run_ids[on_run]),
                    int(all_runs_info[run][0]),
                    on_run_el,
                    off_run_el,
                ]
            ]
            ...
            for off_run in range(0, len(list_no_repeat_off_run_ids)):
                if all_runs_info[run][0] == list_no_repeat_off_run_ids[off_run]:
                    already_used = True
            if not already_used:
...
if input_name == "AUX_files":
    ...
    obs_ra = input_ra
    obs_dec = input_dec
    my_list_on_run_ids = find_runs_near_galactic_plane(
        obs_name, run_epoch, run_obs_type, 10.0, 90.0, 8.0, 20.0, run_elev_range
    )
    my_list_off_run_ids = find_off_runs_around_source(
        obs_name,
        obs_ra,
        obs_dec,
        run_epoch,
    ...
    for lhs in range(0, len(target_lhs_name)):
        ...
        obs_name = (
            target_lhs_name[lhs].replace(" ", "_").replace("+", "_p").replace("-", "_m")
        )
        obs_name += "_%s" % (run_epoch)
        run_elev_range = [input_elev_low, input_elev_up]
        my_list_on_run_ids = find_on_runs_around_source(
            obs_name, obs_ra, obs_dec, run_epoch, run_obs_type, run_elev_range, 2.0
        )
    ...
    my_list_imposter_run_ids = find_off_runs_around_source(
        obs_name,
        obs_ra,
        obs_dec,
        run_epoch,
    ...
    my_list_imposter_off_run_ids = find_off_runs_around_source(
        obs_name,
        obs_ra,
        obs_dec,
        run_epoch,
